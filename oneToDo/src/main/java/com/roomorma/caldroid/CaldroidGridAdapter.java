...
public class CaldroidGridAdapter extends BaseAdapter {
	...
	protected void customizeTextView(int position, TextView cellView) {
		...
		cellView.setTypeface(TypeFaces.get(context, "HelveticaNeueLTStd-Roman.otf"));
		// Get dateTime of this cell
		DateTime dateTime = this.datetimeList.get(position);

		// Set color of the dates in previous / next month
		if (dateTime.getMonth() != month) {
			cellView.setTextColor(Color.parseColor("#4DFFFFFF"));
		}
		...
		if ((minDateTime != null && dateTime.lt(minDateTime))
			...
			if (CaldroidFragment.disabledBackgroundDrawable == -1) {
				...
				cellView.setBackgroundResource(color.active);
			}

			if (dateTime.equals(getToday())) {
				cellView.setBackgroundResource(R.drawable.round_calendar);
			}
		} else {
		...
		if (selectedDates != null && selectedDatesMap.containsKey(dateTime)) {
			if (CaldroidFragment.selectedBackgroundDrawable != -1) {
				...
				cellView.setBackground(resources
						.getDrawable(R.drawable.round_calendar));
			}
